...
const { escapeRegex, getRelativePath, getFilename, getAssetStoragePath } = require('./paths');
const {
  extractHtmlAssets,
...
class WebDownloader {
  ...
  shouldIncludeAsset(fullUrl, pageOrigin) {
    if (this.options.sameOriginOnly) {
      return fullUrl.startsWith(pageOrigin);
    }
  ...
  getStats() {
    return {
      pages: this.downloaded.size,
      assets: this.savedAssets.size,
  ...
  async downloadAsset(fullUrl, pageOrigin) {
    const assetLocalPath = getAssetStoragePath(
      fullUrl,
      pageOrigin,
      this.options.sameOriginOnly
    );
    ...
    try {
      ...
      if (isText && isCssResource(fullUrl, contentType)) {
        fileContent = await this.rewriteCssAssets(
          content,
          new URL(fullUrl),
          assetLocalPath,
          pageOrigin
  ...
  async rewriteCssAssets(css, cssBaseUrl, cssLocalPath, pageOrigin) {
    ...
    const replacements = await mapPool(
      ...
      async (reference) => {
        try {
          ...
          if (!this.shouldIncludeAsset(nestedFullUrl, pageOrigin)) {
            return null;
          }

          const nestedLocalPath = await this.ensureAssetDownloaded(nestedFullUrl, pageOrigin);
  ...
  async downloadAssets(html, baseUrl, pageLocalPath) {
    ...
    const replacements = await mapPool(
      ...
      async (asset) => {
        try {
          const assetLocalPath = await this.ensureAssetDownloaded(asset.fullUrl, pageOrigin);
